...
DEFAULT_MAPPING_JSON = Path(__file__).resolve().parent / "building_nmi_mapping.json"


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Merge an NMI-level wide CSV/parquet into grouped building-level columns.")
    parser.add_argument("input_file", type=Path, help="Path to the NMI-level wide CSV or parquet file.")
    parser.add_argument(
        "--output",
        type=Path,
        default=None,
...
def read_input_table(input_file: Path) -> pd.DataFrame:
    if input_file.suffix.lower() in {".pq", ".parquet"}:
        return pd.read_parquet(input_file)
    return pd.read_csv(input_file)


def merge_file(input_file: Path, mapping_json: Path, output_csv: Path) -> None:
    mapping = load_mapping_json(mapping_json)
    df = read_input_table(input_file)
    input_nmis = get_input_nmis(list(df.columns))
...
def main() -> None:
    ...
    if output_path is None:
        output_path = args.input_file.with_name(f"{args.input_file.stem}_merged_by_building.csv")

    merge_file(args.input_file, args.mapping_json, output_path)
